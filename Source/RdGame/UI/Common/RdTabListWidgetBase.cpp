...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RdTabListWidgetBase)

void URdTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void URdTabListWidgetBase::NativeConstruct()
{
...
void URdTabListWidgetBase::NativeDestruct()
{
	for (FRdTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool URdTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FRdTabDescriptor& OutTabInfo)
{
	const FRdTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FRdTabDescriptor& TabInfo) -> bool
	{
...
void URdTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FRdTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool URdTabListWidgetBase::RegisterDynamicTab(const FRdTabDescriptor& TabDescriptor)
{
...
void URdTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FRdTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void URdTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void URdTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FRdTabDescriptor* TabInfoPtr = nullptr;
	
	FRdTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(URdTabButtonInterface::StaticClass()))
	{
		...
		{
			IRdTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool URdTabListWidgetBase::IsFirstTabActive() const
{
...
bool URdTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool URdTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 URdTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void URdTabListWidgetBase::SetupTabs()
{
	for (FRdTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)

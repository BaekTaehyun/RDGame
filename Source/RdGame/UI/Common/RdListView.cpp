...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RdListView)

#if WITH_EDITOR
#include "Editor/WidgetCompilerLog.h"
#endif

#define LOCTEXT_NAMESPACE "RdListView"

URdListView::URdListView(const FObjectInitializer& ObjectInitializer)
...
void URdListView::ValidateCompiledDefaults(IWidgetCompilerLog& InCompileLog) const
{
...
UUserWidget& URdListView::OnGenerateEntryWidgetInternal(UObject* Item, TSubclassOf<UUserWidget> DesiredEntryClass, const TSharedRef<STableViewBase>& OwnerTable)
{
	TSubclassOf<UUserWidget> WidgetClass = DesiredEntryClass;
	
	for (const URdWidgetFactory* Rule : FactoryRules)
	{
		if (Rule)
		{

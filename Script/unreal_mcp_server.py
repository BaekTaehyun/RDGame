...
def unreal_tick(delta_seconds):
    ...
    while not command_queue.empty():
        ...
        try:
            ...
            # --- New PCG Graph Manipulation Tools ---
            ...
            elif command == "disconnect_pcg_nodes":
                graph_path = payload.get("graph_path")
                up_node_name = payload.get("upstream_node")
                down_node_name = payload.get("downstream_node")
                up_pin = payload.get("upstream_pin", "Out")
                ...
                if not graph:
                    ...
                    if up_node and down_node:
                        try:
                            # Try to remove edge using graph.remove_edge or similar
                            # PCGGraph API: remove_edge(FromNode, FromPinLabel, ToNode, ToPinLabel)
                            removed = graph.remove_edge(up_node, up_pin, down_node, down_pin)
                            if removed:
                                result = {"status": "disconnected"}
                            else:
                                result = {"status": "no_edge_found", "message": "Edge may not exist"}
                        except AttributeError:
                            # Fallback: Try remove_all_incoming_edges on downstream node
                            try:
                                down_node.remove_edge_from(up_pin, up_node, down_pin)
                                result = {"status": "disconnected", "method": "remove_edge_from"}
                            except Exception as e2:
                                result = {"error": f"No remove API found: {e2}"}
                        except Exception as e:
                            result = {"error": str(e)}
                    else:
                        result = {"error": f"Nodes not found. Up:{up_node is not None}, Down:{down_node is not None}"}

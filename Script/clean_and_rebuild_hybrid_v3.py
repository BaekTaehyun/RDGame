...
print("--- [Hybrid] Rebuild V3: Robust Cleanup & Build ---")
...
if graph:
    ...
    for n in graph.nodes:
        ...
        if settings:
            ...
            # Input/Output nodes might not have settings in some versions?
            # Or invalid nodes.
            # Check Node Name for Input/Output
            nm = n.get_name()
            if "Input" in nm or "Output" in nm: keep = True
    ...
    except: graph.add_edge(land_node, "Out", sampler_node, "Surface")
    ...
    print("Hybrid Rebuild V3 Complete.")
...
def rebuild_hybrid_v3():
    print(f"--- [Hybrid] Rebuild V3 ---", flush=True)

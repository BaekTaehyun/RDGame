...
print("=== PCG Cleanup & Restore Landscape ===")
...
if not graph:
    ...
    for node in graph.nodes:
        ...
        # Identify debug nodes created by my scripts
    ...
    # We cannot delete via Python easily in some versions ???
    # Actually graph.remove_node() exists in the API list I printed earlier?
    # No, 'remove_node' IN LIST: ['add_edge', ..., 'remove_node', ...]
    ...
    if debug_nodes:
        ...
        print("No debug nodes found (checked by class/title).")
    ...
    if landscape_node and sampler_node and transform_node and spawner_node:
        ...
        try:
            # Clear old edges just in case? No remove_all_edges.
            # Just add new ones (overrides)
            
            # Landscape -> Sampler
            # Check pins? Landscape usually 'Out', Sampler 'Surface'
        ...
        # Ensure Unbounded is TRUE, otherwise it might only spawn inside the tiny PCG component volume
        try:
            settings = sampler_node.get_settings()
            settings.set_editor_property("unbounded", True)
            # Looseness?
            settings.set_editor_property("looseness", 1.0) # Loose for now
            # PPSM
            settings.set_editor_property("points_per_square_meter", 0.05) # Low density for test
        except: pass
...
def cleanup_and_restore():
    print(f"--- [Cleanup & Restore] ---", flush=True)

...
void UGsNetworkSubsystem::Connect(const FString& Ip, int32 Port, FName SessionName)
{
	// 이미 존재하는 세션인지 확인
	...
	{
		// 없으면 새로 생성
		TUniquePtr<GsNet::FGsNetworkWorker> NewWorker = MakeUnique<GsNet::FGsNetworkWorker>();
		NewWorker->Start();
...
{
	if (TUniquePtr<GsNet::FGsNetworkWorker>* FoundWorker = Workers.Find(SessionName))
	{
		(*FoundWorker)->Disconnect();
		
		// 옵션: 연결 끊으면 워커를 제거할지, 유지할지 결정. 
		// 여기서는 자원 절약을 위해 제거하지 않고 유지하되, 필요 시 명시적 제거 함수 추가 가능.
		// 만약 완전히 제거하고 싶다면:
...
{
	if (const TUniquePtr<GsNet::FGsNetworkWorker>* FoundWorker = Workers.Find(SessionName))
	{
...
void UGsNetworkSubsystem::Send(const TArray<uint8>& PacketData, FName SessionName)
{
	if (TUniquePtr<GsNet::FGsNetworkWorker>* FoundWorker = Workers.Find(SessionName))
	{
		// 스레드로 전달하기 위해 데이터 복사
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Send Failed: Session '%s' not found"), *SessionName.ToString());
	}
}

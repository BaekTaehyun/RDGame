...
{
	Super::BeginPlay();
	// Note: Streaming initialization is typically handled by the Actor or Streamer Component
}

void UDungeonRendererComponent::GenerateDungeon(const FDungeonGrid& Grid, const FDungeonGenConfig& Config, const UDungeonThemeAsset* Theme, UDungeonChunkStreamer* ChunkStreamer)
{
	if (!Theme)
	{
		UE_LOG(LogTemp, Error, TEXT("UDungeonRendererComponent: Missing Theme Asset!"));
		return;
	}
    ...
    if (!FloorHISM)
    {
        FloorHISM = NewObject<UHierarchicalInstancedStaticMeshComponent>(Owner, TEXT("FloorHISM"));
    ...
    if (!CeilingHISM)
    {
        CeilingHISM = NewObject<UHierarchicalInstancedStaticMeshComponent>(Owner, TEXT("CeilingHISM"));
	...
	TileRenderer->GenerateBSPTilesMultiMesh(Grid, Owner, CeilingHISM, FloorHISM, CreatedWallHISMs);
	...
	if (Config.bEnableChunkMerging)
	{
		...
		MergedChunkMeshes = UDungeonMeshMerger::MergeHISMsPerChunk(Owner, ChunkHISMMap, TEXT("MergedDungeon"));
...
void UDungeonRendererComponent::RebuildChunkMaps()
{
    ...
    UE_LOG(LogTemp, Log, TEXT("UDungeonRendererComponent: Rebuilt map with %d chunks."), ChunkHISMMap.Num());
}
...
void UDungeonRendererComponent::HandlePostLoad(bool bIsPIE)
{
    ...
    {
        UE_LOG(LogTemp, Log, TEXT("UDungeonRendererComponent: Found existing walls in PIE. Skipping regeneration."));
        RebuildChunkMaps();
        return;
    }
    
    // Editor or Missing Walls: Do nothing here? 
    // Use Case: If logic says "Regenerate", the Actor should call GenerateDungeon.
    // HandlePostLoad should mainly ensure clean state or restore maps if NOT regenerating.
    // If we ARE regenerating, GenerateDungeon will clear and build.
    // The previous logic forced regeneration in Editor if bWasGenerated was true.
    // Here, the Component doesn't know "bWasGenerated" (Actor state).
    // So HandlePostLoad is just for Cleanup and Map Rebuild.
    // The Actor must decide whether to call GenerateDungeon.
}
